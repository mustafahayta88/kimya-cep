...
class PolymerCanvasView(context: Context) : View(context) {
    ...
    private val chainCountP = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = 0xFFFFA500.toInt(); textSize = 0f; textAlign = Paint.Align.CENTER; isFakeBoldText = true }
    ...
    fun setChainLength(n: Int) { chainLen = n.coerceIn(1, 20); invalidate() }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (polType) {
            0 -> {
                ...
                val ay = h * 0.42f
                c.drawLine(cx - sp * (chainLen * 0.5f), ay, cx - sp * (chainLen * 0.5f), ay + sp * 1.5f, brkP)
                c.drawLine(cx + sp * (chainLen * 0.5f), ay, cx + sp * (chainLen * 0.5f), ay + sp * 1.5f, brkP)

                val chainStartX = cx - (chainLen - 1) * sp * 0.5f
                for (j in 0 until chainLen) {
                    val bx = chainStartX + j * sp
                    da(bx, ay, "C"); da(bx, ay + sp * 0.8f, "C")
                    db(bx, ay, bx, ay + sp * 0.8f)
                    if (j < chainLen - 1) { db(bx + sp * 0.5f, ay, bx + sp, ay); db(bx + sp * 0.5f, ay + sp * 0.8f, bx + sp, ay + sp * 0.8f) }
                }
                chainCountP.textSize = sp * 0.8f
                c.drawText("n = $chainLen", cx, ay + sp * 2.2f, chainCountP)
                c.drawText("Polietilen (PE) - en yaygin plastik", cx, h * 0.82f, capP)
                c.drawText("Kaydirici ile zincir uzunlugunu degistir", cx, h * 0.89f, sP)
                c.drawText("Monomer: Etilen (C2H4) | n: 1000-100000", cx, h * 0.96f, sP)
            }
            1 -> {
            ...
            5 -> {
                c.drawText("Kopolimerler", cx, h * 0.05f, tP)
                c.drawText("Iki farkli monomerin polimerlesmesi", cx, h * 0.11f, lP)

                val my = h * 0.22f
                c.drawText("Blok kopolimer: AAAABBBB", cx, my, sP)
                c.drawText("Asili kopolimer: AAAA", cx - sp * 2f, my + h * 0.08f, sP)
                c.drawText("|", cx - sp * 2f + sp * 0.5f, my + h * 0.09f, sP)
                c.drawText("BBB", cx - sp * 2f + sp * 1.7f, my + h * 0.10f, sP)
                c.drawText("Alternating: ABABAB", cx, my + h * 0.14f, sP)
                c.drawText("Random: ABBAABA", cx, my + h * 0.20f, sP)

                val copY = h * 0.52f
                for (j in 0 until 6) {
                    val bx = cx - sp * 1.5f + j * sp * 0.6f
                    val isB = j % 2 == 1
                    da(bx, copY, if (isB) "B" else "A", if (isB) nAt else cP)
                    if (j < 5) db(bx, copY, bx + sp * 0.6f, copY)
                }
                c.drawText("Alternating kopolimer (en duzenli)", cx, copY + sp * 0.8f, capP)

                val copY2 = h * 0.68f
                for (j in 0 until 6) {
                    val bx = cx - sp * 1.5f + j * sp * 0.6f
                    val isB = j > 2
                    da(bx, copY2, if (isB) "B" else "A", if (isB) nAt else cP)
                    if (j < 5) db(bx, copY2, bx + sp * 0.6f, copY2)
                }
                c.drawText("Blok kopolimer (faz ayrilmasi)", cx, copY2 + sp * 0.8f, sP)

                c.drawText("ABS: Akrilonitril + Butadien + Stiren | Lastik+plastik karisimi", cx, h * 0.88f, sP)
                c.drawText("SBS (stiren-butadien-stiren): termoplastik elastomer", cx, h * 0.94f, sP)
            }
            6 -> {
                c.drawText("Biyobozunur Polimerler", cx, h * 0.05f, tP)
                c.drawText("Dogada mikroorganizmalarla parcalanabilen polimerler", cx, h * 0.11f, lP)

                val ar2 = (w * 0.03f).coerceAtMost(h * 0.04f).coerceAtMost(18f); val sp2 = ar2 * 3f
                val ay = h * 0.30f

                c.drawText("PLA (Poliaktik asit):", cx, ay - sp2 * 0.5f, capP)
                for (j in 0 until 5) {
                    val bx = cx - sp2 * 2f + j * sp2
                    da(bx, ay, "C"); da(bx + sp2 * 0.4f, ay + sp2 * 0.4f, "O", oP); da(bx - sp2 * 0.3f, ay, "C"); da(bx - sp2 * 0.7f, ay + sp2 * 0.3f, "O", oP)
                    db(bx, ay, bx + sp2 * 0.4f, ay + sp2 * 0.4f, 2); db(bx, ay, bx - sp2 * 0.3f, ay); db(bx - sp2 * 0.3f, ay, bx - sp2 * 0.7f, ay + sp2 * 0.3f, 2)
                    if (j < 4) db(bx + sp2 * 0.2f, ay, bx + sp2, ay)
                }
                c.drawText("Misirdan uretilir, kompostta 90 gunde bozunur", cx, h * 0.52f, sP)

                val ay2 = h * 0.58f
                c.drawText("PHA (Polihidroksialkanoat):", cx, ay2 - sp2 * 0.5f, capP)
                for (j in 0 until 5) {
                    val bx = cx - sp2 * 2f + j * sp2
                    da(bx, ay2, "C"); da(bx + sp2 * 0.4f, ay2 + sp2 * 0.3f, "O", oP); da(bx, ay2 - sp2 * 0.3f, "C"); da(bx + sp2 * 0.3f, ay2 - sp2 * 0.3f, "C")
                    db(bx, ay2, bx + sp2 * 0.4f, ay2 + sp2 * 0.3f, 2); db(bx, ay2, bx, ay2 - sp2 * 0.3f); db(bx, ay2 - sp2 * 0.3f, bx + sp2 * 0.3f, ay2 - sp2 * 0.3f)
                    if (j < 4) db(bx + sp2 * 0.3f, ay2, bx + sp2, ay2)
                }
                c.drawText("Bakteri fermentasyonu ile uretilir", cx, h * 0.78f, sP)

                c.drawText("Ornekler: PLA (ambalaj), PHA (medikal), Nisasta bazli (torba)", cx, h * 0.88f, capP)
                c.drawText("Petrol bazli degil, karbon notr, kompostlanabilir | Fiyat: ~2-3x plastik", cx, h * 0.95f, sP)
            }
        }
        canvas.restore()
    }
...
class PolymersFragment : Fragment() {
    ...
    private val categories = listOf("Katilma", "Yogunlasma", "Termoplastik", "Termoset", "Elastomer", "Kopolimer", "Biyobozunur")
    private val details = listOf(
        ...
        "Kopolimerler: Iki veya daha fazla farkli monomerin ayni polimer zincirinde polimerlesmesi. Turleri: Alternating (ABAB), Random (AABBA), Blok (AAA-BBB), Asili/graft (AAAA-BBB). ABS plastik: akrilonitril (sert), butadien (darbe), stiren (parlak). Stiren-butadien-stiren (SBS): termoplastik elastomer.",
        "Biyobozunur Polimerler: Mikroorganizmalar tarafindan CO2 + H2O + biyokutleye parcalanabilen polimerler. PLA (poliaktik asit, misirdan), PHA (bakteri fermentasyonu), nisasta bazli, seluloz bazli. Petrol bazli degil, karbon notr. Kompostlanabilir. Fiyat 2-3x plastik. Medikal (dikis, implant) ve ambalaj."
    )
    private val examples = listOf(
        ...
        "ABS (elektronik govde), SBS (ayakkabi), NBR (hortum), Stiren-akrilonitril (mutfak)",
        "PLA (3D bask iplik), PHA (ambalaj), Nisasta torba, Seluloz film, Kitosan (yara)"
    )
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val chainLabel = TextView(requireContext()).apply {
            text = "Zincir Uzunlugu: 4"; setTextColor(0xFFFFA500.toInt())
            textSize = 14f; setPadding(0, 4, 0, 2)
        }
        val seekBar = android.widget.SeekBar(requireContext()).apply {
            max = 19; progress = 3
            setOnSeekBarChangeListener(object : android.widget.SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(sb: android.widget.SeekBar, p: Int, fromUser: Boolean) {
                    val n = p + 1; chainLabel.text = "Zincir Uzunlugu: $n"
                    if (fromUser) polView.setChainLength(n)
                }
                override fun onStartTrackingTouch(sb: android.widget.SeekBar) {}
                override fun onStopTrackingTouch(sb: android.widget.SeekBar) {}
            })
        }
        parent.addView(chainLabel, idx + 1)
        ...
        categories.forEachIndexed { i, name ->
            Button(requireContext()).apply {
                ...
                setOnClickListener {
                    ...
                    chainLabel.visibility = if (i == 0) android.view.View.VISIBLE else android.view.View.GONE
                    seekBar.visibility = if (i == 0) android.view.View.VISIBLE else android.view.View.GONE
                }
                btnIds.add(this); btnRow.addView(this)
            }
        }
        parent.addView(seekBar, idx + 2)
        v.findViewById<Button>(R.id.btn_help)?.setOnClickListener {
            AlertDialog.Builder(requireContext())
                .setTitle("Polimerler")
                .setMessage("Polimerler, tekrarlayan birimlerden (monomer) olusan buyuk molekullerdir.\n\n" +
                    "Bu bolumde:\n" +
                    "- Polietilen, polipropilen, polistiren turleri\n" +
                    "- Dogal ve sentetik polimerler\n" +
                    "- Zincir uzunlugunu ayarlayarak ozellikleri kesfedebilirsiniz\n" +
                    "- Her polimerin kullanim alani ve yapisi gosterilir\n\n" +
                    "Seeker ile zincir uzunlugunu degistirebilirsiniz.")
